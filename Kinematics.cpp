...
void Kinematics::move() {
  double remainderX = 0.0; // persists between frames
  ...
  // --- Cap velocities ---
  if (kvelocityX > kmaxVel)
    kvelocityX = kmaxVel;
  if (kvelocityX < -kmaxVel)
    kvelocityX = -kmaxVel;
  if (kvelocityY > kmaxVel)
    kvelocityY = kmaxVel;
  if (kvelocityY < -6)
    kvelocityY = -6; // upward cap
  ...
  // Handle collisions
  if (passThisFramePosX && remainderX > 0)
    remainderX = 0;
  if (passThisFrameNegX && remainderX < 0)
    remainderX = 0;

  // Apply position if accumulated movement exceeds threshold
  if (abs(remainderX) >= timeOut) {
